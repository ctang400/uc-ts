...
inline void TsCase::onTrade(const MD::Trade &trade) {
  ...
  if (msg_.cid == Universe::INVALID_CID)
    return;
  // 不按 r1(成交类型)过滤(RPI 等也是真实市场成交, 且模型/sim 数据不区分);
  // 只按合法性静默丢弃 price/qty 非正的 print(半 tick 判断, 浮点不与 0 直接比;
  // 实盘 feed 的非常规 print 带 price=0, 会污染标记价与 trade 信号) ——
  // 与 gexiao 数据转换逻辑一致。
  {
...
void TsCase::onCancelRejected(const oms::ResponseHeader &header,
  ...
  //  只认 binance -2011, 策略侧 reason 恒为 0 无法据此清理本地残单。)
  if (msg.internal_error_code == -3005)
